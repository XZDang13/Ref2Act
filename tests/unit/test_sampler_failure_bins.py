...
def test_adaptive_sampler_disabled_preserves_failure_weighted_probabilities(tmp_path: Path) -> None:
    motion_file = tmp_path / "anchor_motion.npz"
    _write_anchor_sampler_motion_file(motion_file)
    motion_lib = MotionLib([motion_file])
    sampler_mod = _load_sampler_module()
    default_sampler = sampler_mod.Sampler(
        num_envs=1,
        motion_lib=motion_lib,
        dt=0.05,
        anchor_body_index=0,
    ...
    disabled_sampler = sampler_mod.Sampler(
        num_envs=1,
        motion_lib=motion_lib,
        dt=0.05,
        anchor_body_index=0,
        failure_weight_uniform_mix=0.2,
        segment_source=sampler_mod.SegmentSource.Anchor,
        adaptive_sampler=sampler_mod.AdaptiveSamplerCfg(enabled=False),
        device=torch.device("cpu"),
    )

    fail_counts = torch.tensor([25.0, 5.0, 0.0], dtype=torch.float32)
    sample_counts = torch.tensor([30.0, 30.0, 30.0], dtype=torch.float32)
    default_probs = default_sampler._build_guarded_sampling_probabilities(
        fail_counts,
        sample_counts,
        temperature=1.0,
        eligible_mask=default_sampler.bin_reset_eligible[0],
    )
    disabled_probs = disabled_sampler._build_guarded_sampling_probabilities(
        fail_counts,
        sample_counts,
        temperature=1.0,
        eligible_mask=disabled_sampler.bin_reset_eligible[0],
    )

    assert torch.allclose(default_probs, disabled_probs)


def test_adaptive_sampler_quarantines_high_failure_anchor(tmp_path: Path) -> None:
    motion_file = tmp_path / "anchor_motion.npz"
    _write_anchor_sampler_motion_file(motion_file)
    motion_lib = MotionLib([motion_file])
    sampler_mod = _load_sampler_module()
    sampler = sampler_mod.Sampler(
        ...
        anchor_body_index=0,
        segment_source=sampler_mod.SegmentSource.Anchor,
        adaptive_sampler=sampler_mod.AdaptiveSamplerCfg(
            enabled=True,
            warmup_samples=2,
            anchor_drop_fail_rate=0.9,
    ...
    sampler.bin_fail_counts[0][:] = torch.tensor([10.0, 0.0, 0.0])
    sampler.bin_sample_counts[0][:] = torch.tensor([10.0, 10.0, 10.0])
    sampler._update_adaptive_state()

    assert torch.equal(sampler.adaptive_bin_quarantined[0], torch.tensor([True, False, False]))


def test_adaptive_sampler_excludes_quarantined_anchor_from_normal_sampling(tmp_path: Path) -> None:
    motion_file = tmp_path / "anchor_motion.npz"
    _write_anchor_sampler_motion_file(motion_file)
    motion_lib = MotionLib([motion_file])
    sampler_mod = _load_sampler_module()
    ...
    sampler.bin_fail_counts[0][:] = torch.tensor([10.0, 1.0, 1.0])
    sampler.bin_sample_counts[0][:] = torch.tensor([10.0, 10.0, 10.0])
    sampler._update_adaptive_state()
    motion_ids = torch.zeros(256, dtype=torch.long)
    _, target_bin_indices = sampler._sample_failure_weighted_times_for_motion_ids(motion_ids)

    assert bool(sampler.adaptive_bin_quarantined[0][0].item())
    assert not bool(torch.any(target_bin_indices == 0).item())


def test_adaptive_sampler_probe_probability_samples_quarantined_anchor(tmp_path: Path) -> None:
    motion_file = tmp_path / "anchor_motion.npz"
    _write_anchor_sampler_motion_file(motion_file)
    motion_lib = MotionLib([motion_file])
    sampler_mod = _load_sampler_module()
    ...
    assert torch.equal(target_bin_indices, torch.zeros(64, dtype=torch.long))
    assert sampler.get_adaptive_stats()["adaptive_probe_sample_count"].item() == 64.0


def test_adaptive_sampler_downweights_mastered_anchor(tmp_path: Path) -> None:
    motion_file = tmp_path / "anchor_motion.npz"
    _write_anchor_sampler_motion_file(motion_file)
    motion_lib = MotionLib([motion_file])
    sampler_mod = _load_sampler_module()
    sampler = sampler_mod.Sampler(
        ...
        dt=0.05,
        anchor_body_index=0,
        failure_weight_uniform_mix=1.0,
        failure_weight_max_uniform_ratio=None,
        failure_weight_exploration_bonus=0.0,
        segment_source=sampler_mod.SegmentSource.Anchor,
        adaptive_sampler=sampler_mod.AdaptiveSamplerCfg(
            ...
            mastered_probability_scale=0.1,
            probe_probability=0.0,
        ),
        device=torch.device("cpu"),
    )

    sampler.bin_fail_counts[0][:] = torch.tensor([0.0, 10.0, 10.0])
    sampler.bin_sample_counts[0][:] = torch.tensor([100.0, 100.0, 100.0])
    sampler._update_adaptive_state()
    ...
    assert bool(sampler.adaptive_bin_mastered[0][0].item())
    assert counts[0] < counts[1]
    assert counts[0] < counts[2]


def test_adaptive_sampler_quarantines_motion_after_enough_bad_anchors(tmp_path: Path) -> None:
    anchor_times = np.asarray([0.1, 0.25, 0.4, 0.6, 0.8], dtype=np.float32)
    motion_a = tmp_path / "bad_motion.npz"
    motion_b = tmp_path / "good_motion.npz"
    _write_anchor_motion_file(motion_a, anchor_times)
    _write_anchor_motion_file(motion_b, anchor_times)
    motion_lib = MotionLib([motion_a, motion_b])
    sampler_mod = _load_sampler_module()
    sampler = sampler_mod.Sampler(
        num_envs=1,
        ...
        adaptive_sampler=sampler_mod.AdaptiveSamplerCfg(
            ...
            motion_drop_fail_rate=0.9,
            motion_drop_anchor_fraction=0.8,
            min_live_anchor_fraction=0.2,
            min_live_motion_fraction=0.5,
            probe_probability=0.0,
        ),
        device=torch.device("cpu"),
    )

    sampler.bin_fail_counts[0][:] = torch.full((5,), 100.0)
    sampler.bin_sample_counts[0][:] = torch.full((5,), 100.0)
    sampler.bin_fail_counts[1][:] = torch.zeros(5, dtype=torch.float32)
    sampler.bin_sample_counts[1][:] = torch.full((5,), 100.0)
    sampler._update_adaptive_state()

    assert int(sampler.adaptive_bin_quarantined[0].sum().item()) == 4
    assert torch.equal(sampler.adaptive_motion_quarantined, torch.tensor([True, False]))


def test_adaptive_sampler_live_fraction_guards_prevent_sampler_collapse(tmp_path: Path) -> None:
    anchor_times = np.asarray([0.1, 0.25, 0.4], dtype=np.float32)
    motion_a = tmp_path / "bad_motion_a.npz"
    motion_b = tmp_path / "bad_motion_b.npz"
    _write_anchor_motion_file(motion_a, anchor_times)
    _write_anchor_motion_file(motion_b, anchor_times)
    motion_lib = MotionLib([motion_a, motion_b])
    sampler_mod = _load_sampler_module()
    sampler = sampler_mod.Sampler(
        ...
        adaptive_sampler=sampler_mod.AdaptiveSamplerCfg(
            ...
            motion_min_samples=10,
            motion_drop_fail_rate=0.9,
            motion_drop_anchor_fraction=1.0 / 3.0,
            min_live_anchor_fraction=0.6,
            min_live_motion_fraction=0.5,
            probe_probability=0.0,
        ),
        device=torch.device("cpu"),
    ...
    for motion_id in range(2):
        sampler.bin_fail_counts[motion_id][:] = torch.full((3,), 100.0)
        sampler.bin_sample_counts[motion_id][:] = torch.full((3,), 100.0)
    sampler._update_adaptive_state()

    assert int(sampler.adaptive_bin_quarantined[0].sum().item()) == 1
    assert int(sampler.adaptive_bin_quarantined[1].sum().item()) == 1
    assert int(sampler.adaptive_motion_quarantined.sum().item()) == 1
