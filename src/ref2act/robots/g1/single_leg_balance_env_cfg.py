...
@configclass
class G1SingleLegBalanceEnvCfg(DirectRLEnvCfg):
    """Fixed-left single-leg stabilization under a horizontal force pulse."""
    seed = 0
    ...
    pair_action_history = "applied_action"
    task = default_config()
    ...
    # Match G1 locomotion action offsets; the single-leg reset pose is independent.
    robot.init_state.joint_pos.update({
        ".*_hip_pitch_joint": -0.15, ".*_knee_joint": 0.35, ".*_ankle_pitch_joint": -0.20,
    })
